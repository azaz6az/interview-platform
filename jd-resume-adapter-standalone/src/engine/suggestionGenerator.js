...
/**
 * 生成关键词补充建议
 * @param {string[]} missingKeywords - 缺失的关键词列表
 * @returns {Object[]} 建议数组 { type, title, description, original?, suggested? }
 */
 ...
 * @param {string[]} matchedKeywords - 已匹配的关键词
 * @returns {Object[]} 建议数组
 */
function generatePhrasingSuggestions(resumeText, jdText, matchedKeywords) {
  ...
  for (const mapping of phrasingMap) {
    if (mapping.from.test(resumeText)) {
      ...
      mapping.from.lastIndex = 0; // 重置正则 lastIndex
    }
  }
 ...
 * 生成经历优先级调整建议
 * @param {string} resumeText - 简历原文
 * @param {string[]} matchedKeywords - 已匹配关键词
 * @param {string[]} missingKeywords - 缺失关键词
 * @returns {Object[]} 建议数组
 */
function generatePrioritySuggestions(resumeText, matchedKeywords, missingKeywords) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (hasKeyword) {
      keywordLineIndices.push(i);
    }
  }

  // 如果有关键词匹配的行不在前半部分，建议前置
...
/**
 * 生成技能匹配建议
 * @param {Object} matchDetails - 各类别的匹配详情 { tech: { matched, missing }, ... }
 * @param {Object} scoreBreakdown - 各类别分数 { tech, softSkill, data, experience }
 * @returns {Object[]} 建议数组
 */
...
/**
 * 生成修改后的简历
 * @param {string} resumeText - 原始简历
 * @param {string[]} missingKeywords - 缺失的关键词
 * @param {Object[]} suggestions - 所有建议
 * @returns {string} 修改后的简历
 */
function generateModifiedResume(resumeText, missingKeywords, suggestions) {
  let modified = resumeText;

  // 1. 在简历末尾追加缺失关键词对应的技能（如果用户确实掌握的话）
 ...
 * @param {Object} matchDetails - 匹配详情
 * @param {Object} scoreResult - 分数结果 { score, breakdown }
 * @returns {Object} { suggestions, modifiedResume }
 */
